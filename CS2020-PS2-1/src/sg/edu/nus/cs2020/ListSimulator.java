...
public class ListSimulator {
	...
	static public void main(String[] args){
		...
		FixedLengthList fRandom = new MoveToFrontList(LISTSIZE);
		ListSimulator fsim_Random = new ListSimulator(TestType.RANDOM, fRandom);
		
		FixedLengthList fUp = new MoveToFrontList(LISTSIZE);
		ListSimulator fsim_Up = new ListSimulator(TestType.INCREASING, fUp);
		
		FixedLengthList fDown = new MoveToFrontList(LISTSIZE);
		ListSimulator fsim_Down = new ListSimulator(TestType.DECREASING, fDown);
		
		FixedLengthList mRandom = new MyFastList(LISTSIZE);
		ListSimulator msim_Random = new ListSimulator(TestType.RANDOM, mRandom);
		
		FixedLengthList mUp = new MyFastList(LISTSIZE);
		ListSimulator msim_Up = new ListSimulator(TestType.INCREASING, mUp);
		
		FixedLengthList mDown = new MyFastList(LISTSIZE);
		ListSimulator msim_Down = new ListSimulator(TestType.DECREASING, mDown);
		...
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = fsim_Random.chooseRandom();
			fsim_Random.search(key);
		}
		
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = fsim_Up.chooseRandom();
			fsim_Up.search(key);
		}
		
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = fsim_Down.chooseRandom();
			fsim_Down.search(key);
		}
		
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = msim_Random.chooseRandom();
			msim_Random.search(key);
		}
		
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = msim_Up.chooseRandom();
			msim_Up.search(key);
		}
		
		key = 0;
		for (i=0; i<ListSimulator.NUMQUERIES; i++){
			key = msim_Down.chooseRandom();
			msim_Down.search(key);
		}
		...
		System.out.println("Total cost random for MoveToFront: " + fsim_Random.getTotal());
		System.out.println("Total cost increasing for MoveToFront: " + fsim_Up.getTotal());
		System.out.println("Total cost decreasing for MoveToFront: " + fsim_Down.getTotal());
		
		System.out.println("Total cost random for MyFastList: " + msim_Random.getTotal());
		System.out.println("Total cost increasing for MyFastList: " + msim_Up.getTotal());
		System.out.println("Total cost decreasing for MyFastList: " + msim_Down.getTotal());
